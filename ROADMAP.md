...
### 2.4 — Shelter Building and Experience Memory ✅
...
- Shelter building approach: LLM generates code (Approach A) — no hardcoded primitive; `pillarUp` is the emergency fallback ✅
- `isSheltered` observation flag: solid block above + ≥3 solid walls + light source within 5 blocks ✅
- CurriculumAgent proposes shelter construction when afternoon/sunset and not sheltered ✅
- Episodic survival memory: `ckpt/survival/experiences.json` + `voyager/agents/survival_memory.py` ✅
  - Events recorded: death (with inferred cause via `lastDamagingEntity` cache), significant damage (>2 hearts), shelter_built
  - Lessons generated by nano LLM, injected into CurriculumAgent prompt as "Recent survival experiences"
- `bot.on('death')` and `bot.on('entityHurt')` capture events into `recentReactiveEvents` ✅
- `RESUME` env var support in `run.py` for session continuation without vectordb conflicts ✅
...
- Python override fires at `noon`/`sunset` (not night) to propose shelter before dark ✅ (partial)
- `home.json` concept — set after successful shelter build — deferred to full implementation

---

### 2.6 — Death and Respawn Handling 🔄

- Death event captured via `bot.on('death')` in Node.js and recorded in `experiences.json` ✅
- `_isBeingReset` flag prevents hard-reset kills from polluting experiences ✅
- Item recovery logic (return to death coordinates to collect drops) — deferred
- Spawn-point awareness and navigate-home after respawn — deferred
...
> **Current status:** Items 2.1, 2.3 (hostile mobs), and 2.4 (shelter + experience memory) fully complete. 2.2 partially complete (health flee and lava/fire done, fall damage and potions deferred). 2.5 partially complete (day/night override at noon/sunset done, home.json deferred). 2.6 partially complete (death recording done, item recovery deferred). Next: **2.7 Basic Chat Commands** (`lib/chat.js` + `recentChatCommands` in observations).
